      ...
      try {
        ...
        switch (d.topic) {
          case 'controller':
            ...
            if (pwm) {
              const throttle = map(d.data.r2, 0, 1, 1500, 1700) - map(d.data.l2, 0, 1, 0, 300)
              pwm.setPulseLength(15, throttle);
              pwm.setPulseLength(0, map(d.data.j1[0], 0 - d.data.offset, 1 - d.data.offset, 1200, 1800));
            }
          case 'ping':
